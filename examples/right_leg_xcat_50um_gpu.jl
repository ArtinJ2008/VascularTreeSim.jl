...
function write_right_leg_full_summary(path::AbstractString, raw_path, organ_ids_path, log_path,
                                      ...
                                      domain, mask_info, artery_paths, vein_paths,
    ...
    open(path, "w") do io
        println(io, "# XCAT Corrected Right-Leg 50um GPU Full Run")
        println(io)
        ...
        for (idx, artery_path) in enumerate(artery_paths)
            role = idx == 1 ? "growth seed" : "display overlay added after growth"
            @printf(io, "- %s (%s): %d points, %.2f mm length, %.2f-%.2f mm diameter\n",
        ...
        println(io, "- Vein overlay segments added after growth: $(vein_overlay_segments)")
        println(io)
        ...
        println(io, "- Coverage p50 before overlays: $(round(stats.p50 * 10; digits=2)) mm")
        println(io, "- Coverage p95 before overlays: $(round(stats.p95 * 10; digits=2)) mm")
        println(io, "- Coverage max before overlays: $(round(stats.max * 10; digits=2)) mm")
        println(io)
        println(io, "This full run uses the corrected raw-label XCAT vessel extraction. The longest near-muscle `arteries_rleg` path seeds growth. Additional main arteries and veins are appended only after prediction as fixed overlays, so they do not pull predicted branches or create artificial connectors.")
    end
    return path
end
...
function main_right_leg_xcat_50um_gpu()
    ...
    artery_paths = select_longest_paths(artery_paths_near; max_paths=main_vessel_overlays, min_length_cm=8.0)
    isempty(artery_paths) && error("No usable raw-label artery seed paths were selected")
    growth_artery_path = first(artery_paths)
    artery_overlay_paths = artery_paths[2:end]

    tree = make_single_raw_seed_tree(growth_artery_path; terminal_diameter_cm=terminal_cm)
    tree.name = RIGHT_LEG_50UM_TREE_NAME
    ...
    trees = Dict(RIGHT_LEG_50UM_TREE_NAME => tree)
    ...
    println("=" ^ 78)
    println("XCAT corrected right-leg 50um GPU full run")
    println("Started: $(started_at)")
    println("Raw: $(raw_path)")
    println("Output: $(output_dir)")
    println("Terminal diameter: $(terminal_um) um")
    println("Growth seed: $(growth_artery_path.surface)")
    println("Seed diameter target: $(round(tree.root_diameter_cm * 10; digits=3)) mm")
    ...
    println("[artery seeds] selected=$(length(artery_paths)) from $(length(artery_paths_all)) raw-space artery paths ($(length(artery_paths_near)) near-muscle clipped paths)")
    println("[vein overlays] selected=$(length(vein_paths)) from $(length(vein_paths_all)) raw-space vein paths ($(length(vein_paths_near)) near-muscle clipped paths)")
    ...
    stats = growth_stats[RIGHT_LEG_50UM_TREE_NAME]
    ...
    csv_path = joinpath(output_dir, "xcat_right_leg_corrected_50um_segments.csv")
    ...
    write_right_leg_full_summary(summary_path, raw_path, organ_ids_path, log_path,
        ...
        domain, mask_info, artery_paths, vein_paths, artery_overlay_segments,
    ...
    println()
    println("XCAT Corrected Right-Leg 50um Result")
    println("  added predicted branches = $(stats.added)")
    println("  xcat fixed segments shown = $(count(tree.is_xcat))")
    println("  artery overlay segments   = $(artery_overlay_segments)")
    println("  vein overlay segments     = $(vein_overlay_segments)")
